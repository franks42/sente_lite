...
(println "[info] " "=== Phase 6d: Subscription Restoration Test ===")
...
(println "[info] " "Test 1: Connect and subscribe to channels")
((:connect! client))
(Thread/sleep 1500)
...
(println "[info] " "Subscribing to channels: test-channel-1, test-channel-2")
...
(println "[info] " "Subscriptions tracked" {:subscriptions subscriptions-before})

(when (not= #{"test-channel-1" "test-channel-2"} subscriptions-before)
  (println "ERROR:" "FAILED: Should have 2 subscriptions" {:actual subscriptions-before}))

;; Test 2: Publish message and verify reception
(println "[info] " "Test 2: Publish message to test-channel-1")
...
(let [broadcast-result (server/broadcast-to-channel! "test-channel-1"
  ...
  (println "[info] " "Broadcast result" {:result broadcast-result}))

(Thread/sleep 500)

(def messages-before-restart (count (filter #(= "channel-message" (:type %))
                                            @messages-received)))
(println "[info] " "Messages received before restart" {:count messages-before-restart})

(when (< messages-before-restart 1)
  (println "ERROR:" "FAILED: Should have received at least 1 message" {:actual messages-before-restart}))

;; Test 3: Simulate connection loss and verify reconnection
(println "[info] " "Test 3: Connection loss and reconnection")
(reset! messages-received [])
...
;; Restart server
(start-test-server!)
(println "[info] " "Server restarted - waiting for reconnection and subscription restoration")
...
(println "[info] " "State after reconnect" {:state state-after-reconnect})

(def reconnected? (= :open state-after-reconnect))

;; Test 4: Verify subscriptions were restored
(println "[info] " "Test 4: Verify subscriptions restored")
(def subscriptions-after ((:get-subscriptions client)))
(println "[info] " "Subscriptions after reconnect" {:subscriptions subscriptions-after})
...
(println "[info] " "Subscription confirmations received" {:count (count @subscription-results)
                                                       :results @subscription-results})

;; Test 5: Publish message after reconnection and verify reception
(println "[info] " "=== TEST 5: Verify messages received after reconnection ===" {:state ((:get-state client))})
(reset! messages-received [])
(Thread/sleep 1500) ; Give subscription restoration time to complete

(println "[info] " "About to broadcast to channels")

;; Publish to both channels
(println "[info] " "Broadcasting to test-channel-1")
(def broadcast1-result (server/broadcast-to-channel! "test-channel-1"
                                                     {:test "message-after-reconnect-1"
                                                      :timestamp (System/currentTimeMillis)}))
(println "[info] " "Broadcast 1 result" {:result broadcast1-result})
(Thread/sleep 200)

(println "[info] " "Broadcasting to test-channel-2")
(def broadcast2-result (server/broadcast-to-channel! "test-channel-2"
                                                     {:test "message-after-reconnect-2"
                                                      :timestamp (System/currentTimeMillis)}))
(println "[info] " "Broadcast 2 result" {:result broadcast2-result})
(Thread/sleep 1500)

(def messages-after-restart (filter #(= "channel-message" (:type %))
                                    @messages-received))
(def message-count-after (count messages-after-restart))
(println "[info] " "Messages received after restart"
...
(cond
  ...
  (println "[info] " "Phase 6d PASSED: Subscription restoration working correctly"
  ...
  :else
  (println "ERROR:" "Phase 6d FAILED: Subscription restoration not working correctly"
