#!/usr/bin/env bb
;;
;; Multi-Process Test Server (v2 wire format, fixed port for reconnection tests)
;;
;; Usage: bb mp_server_reconnect_v2.bb <test-id> <port> <duration-sec>
;;
...
(defn parse-args [args]
  (when (< (count args) 3)
    (println "Usage: bb mp_server_reconnect_v2.bb <test-id> <port> <duration-sec>")
    (System/exit 1))
  {:test-id (first args)
...
(defn -main [& args]
  (let [{:keys [test-id port duration-sec]} (parse-args args)
    ...
    (println "[server] Starting v2 server for reconnect test:" test-id "on port" port)

    ;; Start server with fixed port and v2 (EDN) wire format
    (server/start-server!
      {:port port
       :host "localhost"
       :wire-format :edn
