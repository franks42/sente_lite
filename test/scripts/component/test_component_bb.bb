#!/usr/bin/env bb
;;; Test component system on Babashka
;;; Run: ./test/scripts/component/test_component_bb.bb
...
(println "=== Component System Tests (Babashka) ===\n")
...
(let [comp (c/make-component :test/basic {:port 8080})]
  ...
  (test-result "has :state atom" (instance? clojure.lang.Atom (:state comp)))
...
(defmethod c/stats :test/custom
  [{:keys [config state]}]
  (merge (c/stats {:state state})  ; Get default stats
         {:custom-metric 42
