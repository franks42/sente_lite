...
(defn parse-args [args]
  (when (< (count args) 6)
    (tel/error! "Usage: mp_client_reconnect_ephemeral.bb <test-id> <client-id> <channel-id> <initial-port> <initial-message-count> <post-reconnect-message-count>")
    (System/exit 1))
  {:test-id (nth args 0)
...
(defn -main [& args]
  (let [{:keys [test-id client-id channel-id initial-port
    ...
    (tel/log! :info "Client starting" {:process-id process-id
                                       :test-id test-id
      ...
      (wsm/create-managed-client
       {:uri (str "ws://localhost:" initial-port "/")
        :on-state-change (fn [old-state new-state]
                           (tel/event! ::state-change {:old old-state
                                                       :new new-state
                                                       :client-id client-id})
                           ...
                           (when (and (= :open old-state)
                                      (not= :open new-state))
                             (tel/event! ::disconnection-detected {:client-id client-id})
                           ...
                           (when (and @is-reconnecting
                                      (= :open new-state))
                             (tel/event! ::reconnection-detected {:client-id client-id})
                             (reset! reconnection-detected true)
                             (reset! is-reconnecting false)))
        :on-message (fn [msg]
                      (tel/event! ::message-received {:msg msg :client-id client-id})
        ...
        :reconnect {:enabled true
                    ...
                    :port-file-fn (fn []
                                    (tel/event! ::reading-port-file
                                                {:test-id test-id
                                                 :client-id client-id})
                                    (let [new-port (mp/read-port test-id 5000)]
                                      (tel/event! ::port-file-read
                                                  {:new-port new-port
                                                   :client-id client-id})
                                      (when (not= new-port initial-port)
                                        (tel/event! ::port-change-detected
                                                    {:old-port initial-port
                                                     :new-port new-port
                                                     :client-id client-id})
    ...
    ;; Connect
    (tel/log! :info "Connecting to server" {:client-id client-id :port initial-port})
    ((:connect! client))
    ...
    (tel/log! :info "Sending initial messages" {:client-id client-id :count initial-message-count})
    (dotimes [i initial-message-count]
      (let [msg {:type :test-message
        ...
        (when ((:send! client) msg-json)
          ...
          (tel/event! ::message-sent {:client-id client-id :sequence i :phase :initial})))
      (Thread/sleep 500))

    ;; Wait for test-complete signal
    (tel/log! :info "Waiting for test-complete signal" {:client-id client-id})
    (try
      (mp/wait-for-ready test-id "test-complete" 30000)
      (tel/log! :info "Test-complete signal received" {:client-id client-id})
      (catch Exception e
        (tel/error! "Timeout waiting for test-complete" {:error (str e) :client-id client-id})))

    ;; If reconnected, send post-reconnect messages
    (when @reconnection-detected
      (tel/log! :info "Sending post-reconnect messages"
      ...
      (dotimes [i post-reconnect-message-count]
        (let [msg {:type :test-message
          ...
          (when ((:send! client) msg-json)
            ...
            (tel/event! ::message-sent {:client-id client-id :sequence i :phase :post-reconnect})))
        (Thread/sleep 500)))
    ...
    (tel/log! :info "Client completed" {:process-id process-id :result result})
