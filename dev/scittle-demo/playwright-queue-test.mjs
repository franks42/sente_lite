#!/usr/bin/env node
/**
 * Playwright automated test for sente-lite queue + client_scittle.cljs
 *
 * This script:
 * 1. Starts the BB test server
 * 2. Starts a static file server for HTML
 * 3. Launches Playwright browser
 * 4. Runs the queue tests
 * 5. Reports pass/fail and exits
 *
 * Usage: node playwright-queue-test.mjs
 */
...
// Configuration
const BB_SERVER_PORT = 1346;
const STATIC_SERVER_PORT = 8766;
...
function startBBServer() {
  return new Promise((resolve, reject) => {
    log('Starting BB queue test server...');

    bbServer = spawn('bb', ['test-queue-server.bb'], {
      cwd: __dirname,
...
function startStaticServer() {
  return new Promise((resolve) => {
    ...
    staticServer = http.createServer((req, res) => {
      let filePath = req.url === '/' ? '/test-queue-scittle.html' : req.url;
...
async function runTests() {
  ...
  try {
    log(`Loading test page: http://localhost:${STATIC_SERVER_PORT}/test-queue-scittle.html`);
    await page.goto(`http://localhost:${STATIC_SERVER_PORT}/test-queue-scittle.html`);
...
async function main() {
  ...
  console.log('  sente-lite Queue Scittle Test');
  console.log('='.repeat(50));
  console.log('');
